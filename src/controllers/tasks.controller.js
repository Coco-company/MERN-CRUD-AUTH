...
export const createTask = async (req,res) => { //1:26:55
    ...
    const newTask = new Task({
        ...
        date,               // date = date;
        user: req.user.id   //01:34:02 Tenemos el user xq el authRequired ya guardó el "user" dentro del "req.user"
    });
    const savedTask = await newTask.save();
    res.json(savedTask);
};

// BUSCAR UNA UNICA TAREA (TASK)   01:27:30
export const getTask = async (req,res) => {
    ...
    //res.json(task) No es necesario que nos devuelva lo eliminado
